...
void MeshPainter::bindObjectAndData(TriMesh *mesh, openGLObject &object, const std::string &texture_image, const std::string &vshader, const std::string &fshader){
    ...
    object.texopen = glGetUniformLocation(object.program, "texopen1");
...
void MeshPainter::drawMesh(glm::mat4 modelMatrix  ,TriMesh* mesh, openGLObject &object, Light *light, Camera* camera, int texopen){
    ...
    glUniform1i(object.texopen,texopen);
...
// 绘制阴影
void MeshPainter::drawShadow(glm::mat4 modelMatrix ,TriMesh* mesh, openGLObject &object, Light *light, Camera* camera, float pos)
{
    // 相机矩阵计算
	camera->updateCamera();
	camera->viewMatrix = camera->getViewMatrix();
    ...
    n = pos+0.01;   //加上0.01为了防止重合，导致颜色显示模糊
	glBindVertexArray(object.vao);
	glUseProgram(object.program);
    // 绘制阴影
	// 阴影投影矩阵
	glm::mat4 shadowProjectMatrix = light->getAnyShadowMatrix(n);
	// 计算阴影的模型变换矩阵。
	modelMatrix = shadowProjectMatrix*modelMatrix;
	// 传递 unifrom 关键字的矩阵数据。
	glUniformMatrix4fv(object.modelLocation, 1, GL_FALSE, &modelMatrix[0][0]);
	glUniformMatrix4fv(object.viewLocation, 1, GL_FALSE, &camera->viewMatrix[0][0]);
	glUniformMatrix4fv(object.projectionLocation, 1, GL_FALSE, &camera->projMatrix[0][0]);
