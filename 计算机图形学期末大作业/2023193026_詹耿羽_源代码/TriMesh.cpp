...
// 立方体生成12个三角形的顶点索引
void TriMesh::generateCube(glm::vec3 color)
{
	// 创建顶点前要先把那些vector清空
	cleanData();
...
void TriMesh::generatePyramid(glm::vec3 color)
{
	// 创建顶点前要先把那些vector清空
	cleanData();

	vertex_positions.push_back(glm::vec3(-0.5,0.0,-0.5));
	vertex_positions.push_back(glm::vec3(0.5,0.0,-0.5));
	vertex_positions.push_back(glm::vec3(0.5,0.0,0.5));
	vertex_positions.push_back(glm::vec3(-0.5,0.0,0.5));   
	vertex_positions.push_back(glm::vec3(0.0,1.0,0.0));
	vertex_colors.push_back(color);
	vertex_colors.push_back(color);
	vertex_colors.push_back(color);
	vertex_colors.push_back(color);
	...
	// 每个三角面片的顶点下标	
	faces.push_back(vec3i(1,4,0));
	faces.push_back(vec3i(0,4,3));
	faces.push_back(vec3i(3,4,2));
	faces.push_back(vec3i(2,4,1));
	faces.push_back(vec3i(0, 1, 2));
	faces.push_back(vec3i(0, 2, 3));
...
void TriMesh::generateGround(glm::vec3 color)
{
	cleanData();
	//顶点坐标及颜色
	vertex_positions.push_back(glm::vec3(-1,0.0,-1));
	vertex_positions.push_back(glm::vec3(1,0.0,-1));
	vertex_positions.push_back(glm::vec3(1,0.0,1));
	vertex_positions.push_back(glm::vec3(-1,0.0,1));   
	vertex_colors.push_back(color);
...
void TriMesh::generateDisk(int num_division, float radius)
{
	...
	{
		float r_r_r = i * step;
		float x = radius * cos(r_r_r)+2*radius;
	...
	//圆盘中心坐标
	vertex_positions.push_back(glm::vec3(2*radius,0,0));
	...
	for (int i = 0; i < num_samples; i++)
	{
		...
		int m = i%num_samples;
		int n = (i+1)%num_samples;

		// 面片对应的顶点的纹理坐标
		vertex_textures.push_back(glm::vec2(0.5,0.5));
		vertex_textures.push_back(glm::vec2((vertex_positions[m].x-radius)/(2*radius), (vertex_positions[m].z+radius)/(2*radius)));
		vertex_textures.push_back(glm::vec2((vertex_positions[n].x-radius)/(2*radius), (vertex_positions[n].z+radius)/(2*radius)));
...
void TriMesh::generateCone(int num_division, float radius, float height)
{
	...
	{
		float r_r_r = i * step;
		float x = radius * cos(r_r_r)+4*radius;
		float y = radius * sin(r_r_r);
		// 添加顶点坐标
		vertex_positions.push_back(glm::vec3(x, y, z));
		vertex_normals.push_back( normalize(glm::vec3(x, y, 0)));
	...
	//圆锥顶点坐标
	vertex_positions.push_back(glm::vec3(4*radius,0,-height));
	vertex_normals.push_back(normalize(glm::vec3(0,0,1)));
	vertex_colors.push_back( normalize(glm::vec3(0,0,1)));
...
glm::mat4 Light::getAnyShadowMatrix(float n)
{
	float lx, ly, lz;
